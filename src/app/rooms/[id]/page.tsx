        ...
        <div
          onClick={() => {
            if (isStellaBattleActive) {
              if (isOwner) router.push(`/rooms/${roomId}/battle/${char.id}`);
            } else {
              router.push(`/rooms/${roomId}/throw/${char.id}`);
            }
          }}
          className={`group relative flex items-center justify-between rounded-2xl bg-white dark:bg-zinc-900/80 p-4 sm:p-5 border shadow-sm transition-all hover:shadow-md cursor-pointer ${isStellaBattleActive
              ? isOwner
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (type === 'CHARACTER_UPDATE') {
          ...
          // リダイレクト（オーバーレイページやバトル参加ページ以外）
          if (!window.location.pathname.includes('/overlay') && !window.location.pathname.includes('/battle')) {
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {roomData.characters.map(char => (
                                      <button
                                        ...
                                        className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all ${!overlayConfig.hiddenCharIds.includes(char.id)
