...
function SortableCharacterItem({
  ...
  isStellaBattleActive: boolean;
  setActiveStellaCharacterId: (id: string | null) => void;
}) {
        ...
        <div
          onClick={() => {
            if (isStellaBattleActive) {
              if (isOwner) setActiveStellaCharacterId(char.id);
            } else {
              router.push(`/rooms/${roomId}/throw/${char.id}`);
            }
          }}
          className={`group relative flex items-center justify-between rounded-2xl bg-white dark:bg-zinc-900/80 p-4 sm:p-5 border shadow-sm transition-all hover:shadow-md cursor-pointer ${
            isStellaBattleActive 
              ? isOwner 
                ? 'border-rose-500 bg-rose-50/20 shadow-rose-500/20' 
...
export default function RoomPage() {
  ...
  const [stellaBattleCoefficient, setStellaBattleCoefficient] = useState(100);
  const [isStartingStellaBattle, setIsStartingStellaBattle] = useState(false);
  const [showStellaConfigModal, setShowStellaConfigModal] = useState(false);
  const [isStellaBattleActive, setIsStellaBattleActive] = useState(false);
  const [activeStellaCharacterId, setActiveStellaCharacterId] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchRoom = async () => {
      try {
        ...
        setStellaBattleCoefficient(data.stellaBattleCoefficient * 100);
      } catch (err) {
        console.error(err);
        ...
        if (type === 'CHARACTER_UPDATE') {
          ...
          setRoomData(prev => {
            ...
            const newChar = {
              ...
              stellaBattleBouquets: data.stella_battle_bouquets || 0
            };
          ...
          setStellaBattleCoefficient(data.coefficient * 100);
          // リダイレクト（オーバーレイページ以外）
...
${overlayConfig.hideName ? '.char-name { display: none !important; }\n' : ''}${overlayConfig.hideCount ? '.char-count-wrapper { display: none !important; }\n' : ''}${overlayConfig.hideImages ? '.char-image-container { display: none !important; }\n' : ''}${overlayConfig.hiddenCharIds.map(id => `.char-${id} { display: none !important; }`).join('\n')}`;
  ...
  const handleStartStellaBattle = async () => {
    setIsStartingStellaBattle(true);
    try {
      const res = await fetch(`/api/rooms/${roomId}/stella-battle/start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ coefficient: stellaBattleCoefficient / 100 }),
      });
      if (!res.ok) throw new Error('開始に失敗しました');
      setShowStellaConfigModal(false);
    } catch (err) {
      alert('エラーが発生しました');
    } finally {
      setIsStartingStellaBattle(false);
    }
  };

  const handleEndStellaBattle = async () => {
    if (!confirm('ステラバトルを終了しますか？')) return;
    try {
      await fetch(`/api/rooms/${roomId}/stella-battle/end`, { method: 'POST' });
    } catch (err) {
      alert('終了に失敗しました');
    }
  };

  const handleUpdateStellaBouquets = async (charId: string, amount: number) => {
    try {
  ...
  return (
    <main className="container mx-auto px-4 py-12 min-h-screen relative pb-32">
      ...
      <div className="max-w-3xl mx-auto">
        {/* 部屋主のみ表示: ステラバトル・編集・保存・キャンセルボタン */}
        {isRoomCreator && (
          <div className="flex flex-col gap-4 mb-6">
            {!isEditMode && (
              <div className="p-1 rounded-3xl border-2 border-rose-500/30 bg-rose-50/10 dark:bg-rose-900/5">
                {isStellaBattleActive ? (
                  <button
                    onClick={handleEndStellaBattle}
                    className="w-full py-4 px-6 rounded-[1.4rem] bg-zinc-800 dark:bg-zinc-100 text-white dark:text-zinc-900 font-black text-lg shadow-xl hover:opacity-90 transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <button
                    onClick={() => setShowStellaConfigModal(true)}
                    className="w-full py-4 px-6 rounded-[1.4rem] border-2 border-rose-500 bg-white dark:bg-zinc-900 text-rose-500 font-black text-lg shadow-xl shadow-rose-500/20 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center gap-2"
                  >
                    <TrendingUp className="w-5 h-5" />
                    ステラバトルを開始する
                  </button>
            ...
            <div className="flex justify-end gap-2">
              {isEditMode ? (
                ...
                <button
                  ...
                  disabled={isStellaBattleActive}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium text-zinc-600 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                >
        ...
        <div className="flex flex-col gap-4">
              ...
              >
                {displayChars.map((char) => (
                  <SortableCharacterItem
                    ...
                    roomData={roomData}
                    isStellaBattleActive={isStellaBattleActive}
                    setActiveStellaCharacterId={setActiveStellaCharacterId}
                  />
                ))}
              </SortableContext>
            </DndContext>
          ...
          {/* キャラ追加カード (全員に表示) */}
          {!isEditMode && !isStellaBattleActive && (
            <div className="mt-4">
              {isAdding ? (
                  ...
                  {roomAllowViewStats && roomData?.overlayToken && (
                    <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800">
                      <button
                        ...
                        className="flex items-center justify-between w-full p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-xl transition-colors group"
                      >
                        <div className="flex items-center gap-2">
                          <ExternalLink className="w-4 h-4 text-rose-500" />
                          <h3 className="text-sm font-bold text-zinc-800 dark:text-zinc-100 group-hover:text-rose-500">配信用オーバーレイ設定</h3>
                        </div>
                        <ChevronDown className={`w-4 h-4 text-zinc-400 transition-transform duration-300 ${isOverlaySettingsOpen ? "rotate-180" : ""}`} />
                      </button>
      ...
      {/* ステラバトル操作パネル */}
      <AnimatePresence>
        {activeStellaCharacterId && roomData && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setActiveStellaCharacterId(null)}
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
            >
              {/* ヘッダー: 自キャラ情報 */}
              {(() => {
                const char = roomData.characters.find(c => c.id === activeStellaCharacterId);
                if (!char) return null;
                ...
                return (
                  <>
                    <div className="p-8 border-b border-zinc-100 dark:border-zinc-800 bg-rose-500 text-white flex justify-between items-start">
                      <div className="flex items-center gap-6">
                        <div className="w-24 h-24 rounded-2xl bg-white/20 backdrop-blur-md overflow-hidden border-2 border-white/30 shadow-lg">
                          {char.avatarUrl ? (
                            <div className={`grid w-full h-full ${char.avatarUrl.split(",").filter(u => u.trim()).length > 1 ? "grid-cols-2" : "grid-cols-1"}`}>
                              {char.avatarUrl.split(",").filter(u => u.trim()).map((url, i) => (
                                <img key={i} src={url} alt={char.name} className="w-full h-full object-cover" />
                              ))}
                            </div>
                          ) : (
                        ...
                        <div>
                          <p className="text-xs font-black uppercase tracking-[0.2em] opacity-80 mb-1">Your Character</p>
                          <h2 className="text-4xl font-black font-serif tracking-tight">{char.name}</h2>
                          <div className="mt-2 inline-flex items-center px-3 py-1 bg-white/20 rounded-full text-xs font-bold backdrop-blur-sm">
                            <TrendingUp className="w-3 h-3 mr-1.5" />
                            Stella Battle Mode
                          </div>
                        </div>
                      </div>
                      <button onClick={() => setActiveStellaCharacterId(null)} className="p-2 hover:bg-white/20 rounded-full transition-colors">
                        <X className="w-8 h-8" />
                      </button>
                    </div>
                      ...
                      <div className="p-8 space-y-10">
                        ...
                        <div className="text-center space-y-2">
                          ...
                          <div className="text-8xl font-black font-serif text-rose-500 tabular-nums tracking-tighter">
                            {char.stellaBattleBouquets?.toLocaleString() || 0}
                          </div>
                        </div>
                        ...
                        <div className="bg-zinc-50 dark:bg-zinc-800/50 rounded-[2rem] p-6 space-y-6 border border-zinc-100 dark:border-zinc-800 shadow-inner">
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            {[3, 4, 5].map(val => (
                              <button
                                key={val}
                                onClick={() => handleUpdateStellaBouquets(char.id, -(val * multiplier))}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (!isNaN(val) && val > 0) {
                                  handleUpdateStellaBouquets(char.id, val);
                                  setPlusValue("");
                                }
                              }}
                        ...
                        <div className="space-y-4">
                          <h3 className="text-sm font-black text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                            <Users className="w-4 h-4" /> 全キャラの状況
                          </h3>
                          <div className="grid grid-cols-1 gap-2">
                            {roomData.characters.map(c => (
                              <div key={c.id} className={`flex items-center justify-between p-3 rounded-2xl border transition-all ${c.id === char.id ? 'bg-rose-50 dark:bg-rose-900/10 border-rose-200 dark:border-rose-900/30 shadow-sm' : 'bg-white dark:bg-zinc-900 border-zinc-100 dark:border-zinc-800'}`}>
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className={`font-bold ${c.id === char.id ? 'text-rose-600 dark:text-rose-400' : 'text-zinc-600 dark:text-zinc-300'}`}>{c.name}</span>
                                </div>
                                <span className={`text-xl font-black font-serif tabular-nums ${c.id === char.id ? 'text-rose-500' : 'text-zinc-400'}`}>
      ...
      {/* ステラバトル設定モーダル */}
      <AnimatePresence>
        {showStellaConfigModal && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setShowStellaConfigModal(false)}
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 10 }}
              className="relative w-full max-w-md bg-white dark:bg-zinc-900 rounded-[2.5rem] shadow-2xl p-8 border border-zinc-100 dark:border-zinc-800"
            >
              <div className="text-center space-y-6">
                <div className="w-20 h-20 bg-rose-500 rounded-3xl flex items-center justify-center mx-auto shadow-lg shadow-rose-500/30">
                  <TrendingUp className="w-10 h-10 text-white" />
                </div>
                
                <div className="space-y-2">
                  <h3 className="text-2xl font-black text-zinc-800 dark:text-zinc-100 font-serif">ステラバトル設定</h3>
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">バトルの初期値を決定する係数を設定してください。</p>
                </div>

                <div className="bg-zinc-50 dark:bg-zinc-800/50 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                  <label className="block text-xs font-black text-zinc-400 uppercase tracking-widest mb-3">初期値係数</label>
                ...
                <div className="space-y-3">
                  <button
                    onClick={handleStartStellaBattle}
                    disabled={isStartingStellaBattle}
                    className="w-full py-4 bg-rose-500 text-white font-black rounded-2xl shadow-xl shadow-rose-500/30 hover:bg-rose-600 active:scale-95 transition-all flex items-center justify-center gap-2"
                  >
                    {isStartingStellaBattle ? <Loader2 className="w-6 h-6 animate-spin" /> : "バトル開始"}
                  </button>
                  <button
                    onClick={() => setShowStellaConfigModal(false)}
                    className="w-full py-3 text-zinc-500 font-bold hover:text-zinc-800 dark:hover:text-zinc-100 transition-colors"
                  >
                    キャンセル
                  </button>
                </div>
                ...
                <p className="text-[10px] text-zinc-400 leading-relaxed">
                  ※開始すると、この部屋の全ユーザーに通知が届き、<br />
                  強制的にルームページにリダイレクトされます。
                </p>
              </div>
            </motion.div>
          </div>
